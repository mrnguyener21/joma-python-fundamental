...
# # The result matrix, known as the matrix product, has the number of rows of the first and the number of columns of the second matrix. 

# #There is a formal definition of matrix multiplication on wikipedia. https://en.wikipedia.org/wiki/Matrix_multiplication
# #Output "None" if the matrices cannot be multiplied together. 
# # Use the previous function is_valid_matrix to validate the two matrices before you multiply them 

# def create_matrix(num_of_rows, num_of_cols):
#     #this will create a 0 matrix with certain num of rows and cols
#     my_matrix = []
...
#     if not is_valid_matrix(m) or not is_valid_matrix(n):
#         return None
 
#     num_of_rows1 = len(m)
#     num_of_cols1 = len(m[0])
 
#     num_of_rows2 = len(n)
#     num_of_cols2 = len(n[0])
 
#     if num_of_cols1 != num_of_rows2:
#         return None

#     answer = create_matrix(num_of_rows1, num_of_cols2)

#     for i in range( num_of_rows1 ):
#         for j in range( num_of_cols2 ):
#             for k in range( num_of_rows2 ):
...
#Given a list, determine if the list is in increasing order

def is_increasing(lst):
    # STEP 1: Find the Base case: [] and len(lst) == 1
    if len(lst) < 2: #basically, if lst == [] or len(lst) == 1:
        return True
    # STEP 2: Find simplier problem: 
    # is the first element bigger than the second one?
    elif lst[0] < lst[1]:
...
#RECURSION PROBLEM 3 : FIND THE INTERSECTION OF TWO LISTS
#this is the same question as to the assignment for loops problem 2, however, this time you cannot use loops or variables.